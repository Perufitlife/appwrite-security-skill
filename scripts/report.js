...
function findingCard(f, idx) {
  ...
  <div class="${style.bg} ${style.text} border-l-4 ${style.border} p-5 rounded shadow-sm mb-3">
    ...
    <details class="text-xs">
      <summary class="cursor-pointer font-semibold text-green-800">Fix SQL (copy & run in Appwrite SQL editor)</summary>
...
export function renderHtml(result) {
  const { appwrite_endpoint, appwrite_project, scanned_at, summary, findings, n_databases, n_collections, active_probe } = result;
  const project_name = `${appwrite_project} @ ${appwrite_endpoint}`;
  const project_ref = appwrite_project;
  const region = "Appwrite instance";
  const n_tables_scanned = n_collections;
  const n_functions_scanned = n_databases;
...
<head>
  <meta charset="UTF-8">
  <title>Appwrite Security Report — ${escapeHtml(project_name)}</title>
...
<body class="bg-gray-50 text-gray-900 font-sans">
  <div class="max-w-5xl mx-auto p-6">
    ...
    <div class="bg-gradient-to-r from-emerald-700 to-teal-600 text-white p-8 rounded-lg shadow-lg mb-6">
      <h1 class="text-3xl font-bold mb-2">Appwrite Security Report</h1>
    ...
    <div class="bg-white p-5 rounded-lg shadow mb-6">
      ...
      <div class="grid grid-cols-3 gap-4 text-center">
        <div><div class="text-2xl font-bold text-gray-700">${n_tables_scanned}</div><div class="text-xs text-gray-500">Collections scanned</div></div>
        <div><div class="text-2xl font-bold text-gray-700">${n_functions_scanned}</div><div class="text-xs text-gray-500">Databases scanned</div></div>
        <div><div class="text-2xl font-bold text-gray-700">5</div><div class="text-xs text-gray-500">Permission types checked</div></div>
      </div>
    </div>
    ...
    <div class="bg-green-50 border-l-4 border-green-500 text-green-900 p-6 rounded-lg shadow mb-6 text-center">
      ...
      <p class="mt-2">Your Appwrite project passes all checks.</p>
    </div>
    `}
    ...
    <div class="bg-white p-5 rounded-lg shadow mb-6">
      ...
      <p class="text-sm text-gray-600 mb-3">Copy and run in Appwrite Dashboard → SQL Editor. Review each statement before executing.</p>
    ...
    <div class="text-center text-xs text-gray-500 mt-8 pb-4">
      Generated by <a href="https://github.com/Perufitlife/appwrite-security-skill" class="text-emerald-700 underline">appwrite-security</a>
